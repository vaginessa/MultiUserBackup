...
public class AppInfoHelper {
    ...
    public static ArrayList<AppInfo> getPackageInfo(Context context, File backupDir,
                                                    boolean includeUnistalledBackups, ShellCommands shellCommands) {
        ...
        for (PackageInfo pinfo : pinfoList) {
            ...
            if (backupDir != null) {
                ...
                AppInfo appInfo = new AppInfo(pinfo.packageName,
                        ...
                        true);
                appInfo.addUser("0");
                Bitmap icon = getIcon(pm, pinfo);
                appInfo.icon = icon;
                LogFile logFile = extractLogFile(backupDir, appInfo);
                if (logFile != null)
        ...
        Map<String, AppInfo> existsMap = new HashMap<>();
        for (AppInfo i : list) {
            existsMap.put(i.packageName, i);
        }

        List<AppInfo> other = shellCommands.getPackagesOfOtherUsers(context, existsMap, pm);
        for (AppInfo appInfo : other) {
            LogFile logFile = extractLogFile(backupDir, appInfo);
            if (logFile != null)
                appInfo.setLogInfo(logFile);
            list.add(appInfo);
    ...
    private static LogFile extractLogFile(File backupDir, AppInfo appInfo) {
        File subdir = new File(backupDir, appInfo.getPackageName());
        if (subdir.exists()) {
            return new LogFile(subdir, appInfo.getPackageName());
        } else {
            return null;
        }
    }

    static Bitmap getIcon(PackageManager pm, PackageInfo pinfo) {
        Bitmap icon = null;
        Drawable apkIcon = pinfo.applicationInfo.loadIcon(pm);
        try {
